...
class ValidationConfig(BaseModel):
    ...
    strategy: ValidationStrategy = Field(
        default=ValidationStrategy.JWT_LOCAL,
        description="The validation strategy to use"
    )
    fallback_timeout_seconds: float = Field(
        default=5.0,
        description="Timeout for fallback introspection requests"
    )
    periodic_check_interval: int = Field(
        default=300,
        description="Interval for periodic introspection checks in seconds"
    )
    ...
    cache_introspection_results: bool = Field(
        default=True,
        description="Whether to cache introspection results"
    )
    cache_ttl_seconds: int = Field(
        default=60,
        description="TTL for cached introspection results in seconds"
    )
...
class AuthMetrics(BaseModel):
    ...
    average_validation_time_ms: float = Field(default=0.0, description="Average validation time in ms")
