        ...
        if claims:
            for claim in claims:
                if claim in userinfo and claim not in [
                    "given_name", "family_name", "sub", "email", "preferred_username",
                    "realm_access", "resource_access", "roles", "scope"
                ]:
...
class KeycloakBackend(AuthenticationBackend):
    ...
    def __init__(
        ...
        self.cache = TTLCache(
            maxsize=1000, 
            ttl=self.validation_config.cache_ttl_seconds
        )
    ...
    def _record_validation_time(self, validation_time_ms: float) -> None:
        ...
        if self._validation_times:
            self.metrics.average_validation_time_ms = sum(self._validation_times) / len(self._validation_times)
    ...
    async def _validate_introspection_only(self, token: str, token_hash: str) -> EnhancedFastApiUser:
    ...
    async def _validate_jwt_with_periodic_check(self, token: str, token_hash: str) -> EnhancedFastApiUser:
    ...
    async def _background_introspection_check(self, token: str, token_hash: str) -> None:
        ...
        try:
            ...
            if not userinfo.get('active', False):
                log.warning(f"Token {token_hash[:8]}... is no longer active according to introspection")
    ...
    async def get_user_roles(self, user_id: str) -> list[str]:
        ...
        try:
            ...
            client_roles = await self.keycloak_openid.a_get_client_roles_of_user(
                user_id, 
                self.keycloak_configuration.client_id
            )
    ...
    async def assign_role_to_user(self, user_id: str, role_name: str, is_realm_role: bool = True) -> bool:
        """
    ...
    async def remove_role_from_user(self, user_id: str, role_name: str, is_realm_role: bool = True) -> bool:
        """
